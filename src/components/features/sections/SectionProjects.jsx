...
export default function SectionProjects({ mobile, edit, section, onUpdateContent }) {
      ...
      >
        {projects?.map((project, key) => {
          const backgroundImage = project?.images?.length ? {
            ...
            backgroundSize: `100% ${mobileSm || mobile?.mobileSm ? "100%" : ""}`,
          } : {};

          const firstWord = project.title.split(" ")?.[0] || "";
          return (
            <SwiperSlide key={key}>
              <div className="h-[700px] @xs/mobile:!h-[700px] sm:h-[500px] lg:px-4 px-5 @xs/mobile:!px-5 lg:h-[700px] relative object-cover" style={backgroundImage}>
                <div className="absolute z-10 bg-black opacity-60 inset-0 "></div>
                <div className={cn("flex  justify-center flex-col  max-w-[1200px] text-white z-50 relative items-center h-full mx-auto",
                  "@xs/mobile:!justify-center @xs/mobile:items-start @xs/mobile:!flex-col",
                  ...
                  <div className="flex flex-col gap-y-3  max-w-[500px] shrink-0">
                    ...
                    <span data-first-word={firstWord} className="text-xl @xs/mobile:text-2xl sm:text-2xl lg:text-4xl pr-5 font-jasans tracking-wide">{project.title}</span>
                    <div className="max-w-[600px] @xs/mobile:block lg:hidden">
                      <span className="font-jasans @xs/mobile:text-sm lg:text-base text-sm tracking-wider leading-7 line-clamp-[10]">{parser(project?.description || "")}</span>
                    </div>
                  ...
                  <div className="max-w-[600px] @xs/mobile:hidden lg:block hidden">
